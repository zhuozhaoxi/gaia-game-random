...
"""Compress map PNG assets to at most 20,000 bytes with adaptive quality."""
...
def compression_profile(filename: str, source_width: int) -> tuple[int, tuple[int, ...]]:
    if re.fullmatch(r"(?:0[1-9]|10)-实心\.png", filename):
        return 300, (320, 300, 280, 260, 240, 220, 200, 180, 160, 140, 120)
    if re.fullmatch(r"1[1-8]-(?:实心|空心)\.png", filename):
        return 160, (370, 350, 330, 310, 290, 270, 250, 230, 210, 190, 170, 160, 140, 120, 100)
    return 80, (159, 150, 140, 130, 120, 110, 100, 90, 80, 72, 64)
...
def compress(path: Path) -> tuple[int, int, int, int, float] | None:
    ...
    display_width, widths = compression_profile(path.name, source.width)
...
def main() -> None:
    ...
    oversized = [path for path in paths if path.stat().st_size > MAX_BYTES]
    if oversized:
        names = ", ".join(path.name for path in oversized)
        raise RuntimeError(f"仍有图片超过 {MAX_BYTES} 字节：{names}")
