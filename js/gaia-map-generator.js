(function () {
    ...
    function placedR2Cells(sector) {
        ...
        return radiusKeys(2).map(localKey => {
            ...
            const [rotatedQ, rotatedR] = rotateAxial(localQ, localR, sector.rotation);
            return {
                q: sector.q + rotatedQ,
                r: sector.r + rotatedR,
    ...
    function planetConflicts(sectors) {
        ...
        sectors.forEach(sector => placedR2Cells(sector).forEach(cell => {
            if (cell.planet) planets.set(key(cell.q, cell.r), cell);
        }));
        const conflicts = [];
        planets.forEach(cell => DIRECTIONS.forEach(([dq, dr]) => {
            const neighborKey = key(cell.q + dq, cell.r + dr);
            const neighbor = planets.get(neighborKey);
            if (neighbor && neighbor.planet === cell.planet && key(cell.q, cell.r) < neighborKey) {
    ...
    function solveRotations(sectors) {
        ...
        function place(position) {
            ...
            for (const rotation of shuffle([0,1,2,3,4,5])) {
                ...
                planets.forEach(cell => assigned.set(key(cell.q, cell.r), cell.planet));
                if (place(position + 1)) return true;
                planets.forEach(cell => assigned.delete(key(cell.q, cell.r)));
            }
            return false;
        }
        return place(0);
    ...
    function createTriangleLayout() {
        return shuffle(['11','12','13','14','15','16','17','18']).map((number, index) => ({
            number,
            cells: TRIANGLE_SLOTS[index].cells,
    ...
    function usedCoordinates(layout) {
        const used = new Map();
        layout.r2.forEach(sector => placedR2Cells(sector).forEach(cell => used.set(key(cell.q, cell.r), [cell.q, cell.r])));
    ...
    function validate(layout) {
        ...
        const fleets = layout.specials.filter(cell => FLEET_IMAGES.includes(cell.image));
        return {
            valid: layout.r2.length === 10 && new Set(layout.r2.map(sector => sector.number)).size === 10 &&
                ...
                fleets.length === 4 && fleets.every((fleet, index) => fleets.slice(index + 1).every(other => distance(fleet.q-other.q, fleet.r-other.r) > 3)),
            planetConflicts: planetConflicts(layout.r2).length,
            centerNumbers
        };
    }

    window.GaiaMapGenerator = {generate, render, validate};
})();
